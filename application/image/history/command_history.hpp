...
public:
  ...
  void Redo() {
    if (canRedo()) {
      ...
      auto* remove_cmd = static_cast<filter::RemoveFilterCommand*>(pair.undo_command.get());
      ...
      if (remove_cmd && remove_cmd->HasRemovedFilter()) {
        ...
        auto new_remove_command = std::make_unique<filter::RemoveFilterCommand>(receiver);
